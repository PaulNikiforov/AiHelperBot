...
import com.nikiforov.aichatbot.config.properties.RagProperties;
...
public class DocumentRetriever {
    ...
    private List<Document> retrieveForRemindPeers(String query, int maxDocuments) {
        ...
        List<Document> merged =
                documentRanker.mergeWithRemindSubmissionPage(candidates, remindPage, maxDocuments + CANDIDATE_BUFFER);
        log.info("Found {} merged documents for REMIND_PEERS query", merged.size());
    ...
    private List<Document> retrieveForGrowthPlan(String query, int maxDocuments) {
        ...
        List<Document> merged = mergeWithConsecutivePages(candidates, startPage, 4, maxDocuments + CANDIDATE_BUFFER);
        log.info("Found {} merged documents for GROWTH_PLAN query", merged.size());
