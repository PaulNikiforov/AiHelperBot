package com.nikiforov.aichatbot.service.rag;
...
public class DocumentRanker {
    ...
    /**
     * Merges documents with remind-submission page documents, removing duplicates. Remind documents
     * are prepended to the list.
     *
     * @param documents the base documents
     * @param remindPage the page number containing remind-submission section
     * @param maxDocuments maximum number of documents to return
     * @return merged list with remind-submission documents prioritized
     */
    public List<Document> mergeWithRemindSubmissionPage(
            List<Document> documents, int remindPage, int maxDocuments) {
        List<Document> remindDocs = indexer.findByPageNumber(remindPage);

        return Stream.concat(remindDocs.stream(), documents.stream())
                .distinct()
                .limit(maxDocuments)
                .toList();
    }
